<template>
  <HeaderOne
    :menu="menu"
    :header-height="screen_width > 959 ? 150 : 78"
    header-active-link-variant="bottom_line"
    :buttons="buttons"
  >
    <template #header-top-bar>
      <div class="d-flex" v-if="screen_width > 959">
        <v-container class="py-0">
          <div class="d-flex border-b border-grey-darken-1">
            <div class="mx-n4">
              <v-tabs
                v-model="tab"
                height="58"
              >
                <v-tab
                  ...
                  :key="tab_item.id"
                  class="text-body-1 text-grey-darken-2 mx-4 px-0"
                  min-width="0"
                  :value="tab_item.title"
                >
                  {{ tab_item.title }}
                </v-tab>
              </v-tabs>
    ...
    <template #menu>
      <v-window v-model="tab">
        <v-window-item
          v-for="menu_item in tabs"
          :key="menu_item.id"
          :value="menu_item.title"
        >
          <ul class="header-menu flex-grow-1">
            <li v-for="menu_sub in menu_item.list" :key="`header-menu-item-${menu_sub.id}`">
              <NuxtLink
    ...
    <template #header-top-bar-mobile>
      <v-expansion-panels variant="accordion">
        <v-expansion-panel class="px-0" v-for="tab in tabs" :key="tab.id" :title="tab.title" elevation="0">
          <v-expansion-panel-text v-for="menu_item in tab.list" :key="menu_item.title">
            <NuxtLink
              class="text-decoration-none text-grey-darken-2 text-body-2 px-4"
              active-class="header-menu-link-active"
              :to="menu_item.link"
            >
              {{ menu_item.title }}
      ...
      <div class="d-flex flex-column">
        <v-btn
          v-for="link in topLinks"
          :key="link.id"
          class="header-top-right-item d-flex px-6 py-6 justify-start text-body-2 text-grey-darken-1 mb-0 d-inline-block"
          variant="text"
          color="grey-darken-2"
          :prepend-icon="link.prepend_icon"
          :append-icon="link.append_icon"
